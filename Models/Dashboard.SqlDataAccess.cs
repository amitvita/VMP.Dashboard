...
namespace VMP.Dashboard.Models
{
    ...
    public class RevenueOverTimeGrid
    {
        public DateTime Month { get; set; }
        public Nullable<double> CurrentSeriesValue { get; set; }
        public Nullable<double> PreviousSeriesValue { get; set; }
        public Nullable<double> Growth { get; set; }
        public Nullable<double> GrowthAbs { get; set; }
        public string Error { get; set; }
    }
    ...
    public class SqlDataAccess
    {
        ...
        public void FilterCreation(string[] Category, string[] region, string[] segment, string selectedgroupby)
        {
            Filter = "";
            if(Category != null)
            {
        ...
        public List<RevenueOverTime1> GetRevenueOverTimes()
        {
            ...
            //try
            //{
                return Helper.RawSqlQuery("select c."+ GroupByColumn + ", c.CurrentRevenue, p.Prev from (" +
                "(select row_number() over(order by "+ GroupByColumn + ") as id, sum(Sales) as CurrentRevenue, "+ GroupByColumn + "" +
                " from [VMP.Dashboard].[dbo].[Orders] where [Order Date] between '" + StartDatesql + "' and '" + EndDatesql + "' " + Filter + "group by "+ GroupByColumn + "" +
                ")c inner join ( select row_number() over(order by "+ GroupByColumn + ") as id, sum(Sales) as Prev, "+ GroupByColumn + "" +
                " from [VMP.Dashboard].[dbo].[Orders] where [Order Date] between '" + PrevStartDatesql + "' and '" + PrevEndDatesql + "' " + Filter + " group by "+ GroupByColumn + " ) p on c.id=p.id " +
                ") order by "+ GroupByColumn + "", x => new RevenueOverTime1 { Month = (DateTime)x[0], CurrentSeriesValue = x[1] == DBNull.Value ? 0 : (Nullable<double>)x[1], PreviousSeriesValue = x[2] == DBNull.Value ? 0 : (Nullable<double>)x[2], Error = null }).ToList();

            //}
            //catch (Exception ex)
            //{
            //     return new List<RevenueOverTime1>{ new RevenueOverTime1() { Error = ex.ToString() } };
            //}
        ...
        public List<RevenueOverTimeGrid> GetRevenueOverTimesGrid()
        {

            return Helper.RawSqlQuery(@"select c." + GroupByColumn + @", c.CurrentRevenue, p.Prev, Round(ISNULL((c.CurrentRevenue/NULLIF(p.Prev,0))-1,1) *100,2) as Growth, (c.CurrentRevenue - p.Prev) as GrowthAbs
                                        from(
                                        (select row_number() over(order by " + GroupByColumn + @") as id, sum(Sales) as CurrentRevenue, " + GroupByColumn + @"
                                        from [VMP.Dashboard].[dbo].[Orders] where[Order Date] between '" + StartDatesql + @"' and '" + EndDatesql + @"' " + Filter + @" group by " + GroupByColumn + @"
                                        )c inner join(select row_number() over(order by " + GroupByColumn + @") as id, sum(Sales) as Prev, " + GroupByColumn + @"
                                    ...
                                    x => new RevenueOverTimeGrid { Month = (DateTime)x[0], CurrentSeriesValue = x[1] == DBNull.Value ? 0 : (Nullable<double>)x[1], PreviousSeriesValue = x[2] == DBNull.Value ? 0 : (Nullable<double>)x[2], Error = null, Growth = x[3] == DBNull.Value ? 0 : (Nullable<double>)x[3], GrowthAbs = x[4] == DBNull.Value ? 0 : (Nullable<double>)x[4] }).ToList();
        ...
        {
            return Helper.RawSqlQuery("Select Category,MonthDate,Sum(Sales) as Sales from dbo.Orders where [Order Date] between '2019-01-01' and '2019-12-31' group by Category,MonthDate",
                                      x => new CategorySale { Month = (DateTime)x[1], Category = (string)x[0], Sales = x[2] == DBNull.Value ? 0 : (double)x[2] }).ToList();
        }

        public List<CategorySaleBar> GetCategorySaleBars()
        {
            return Helper.RawSqlQuery(@"select PA.Category,SUM(PA.Sales)/ SUM(SUM(PA.Sales)) OVER () AS Percentage  from [VMP.Dashboard].[dbo].[Orders] PA where [Order Date] between '" + StartDatesql + "' and '" + EndDatesql + "' " + Filter + @"
                                      group by PA.Category", x => new CategorySaleBar { Category = (string)x[0], Sales = x[1] == DBNull.Value ? 0 : (double)x[1] }).ToList();
        }
        public List<BrandsBar> GetBrandsBars()
        {
            return Helper.RawSqlQuery(@"select PA.[Sub-Category],SUM(PA.Sales)/ SUM(SUM(PA.Sales)) OVER () AS Percentage from [VMP.Dashboard].[dbo].[Orders] PA where [Order Date] between '" + StartDatesql + "' and '" + EndDatesql + "' " + Filter + @"
                                      group by PA.[Sub-Category]", x => new BrandsBar { Brands = (string)x[0], Sales = x[1] == DBNull.Value ? 0 : (double)x[1] }).ToList();
        }
        public List<RegionBar> GetRegionBars()
        {
            return Helper.RawSqlQuery(@"SELECT PA.Region,SUM(PA.Sales)/ SUM(SUM(PA.Sales)) OVER () AS Percentage from [VMP.Dashboard].[dbo].[Orders] PA where [Order Date] between '" + StartDatesql + "' and '" + EndDatesql + "' " + Filter + @"
                                      group by PA.Region", x => new RegionBar { Region = (string)x[0], Sales = x[1] == DBNull.Value ? 0 : (double)x[1] }).ToList();
        }

        public List<OrderPivot> GetOrderPivots()
        {
            return Helper.RawSqlQuery(@"select Category,[Sub-Category],[Product Name],Sales,[MonthDate] from [VMP.Dashboard].[dbo].[Orders] where [Order Date] between '" + StartDatesql + "' and '" + EndDatesql + "' " + Filter + @"",
                x => new OrderPivot { Category = (string)x[0], Brands = (string)x[1], Products = (string)x[2], sales = x[3] == DBNull.Value ? 0 : (double)x[3], orderdate = (DateTime)x[4] }).ToList();
        }
            ...
            using (var context = new DashboardContext())
            {
                using (var command = context.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        return new List<T>();
                        //entities.Add();
                        //return new List<T> { T.Error = ex.ToString() };
                    }
                }
            }
        }
